...
def verify_mono_t2() -> None:
    ...
    if diff_t2.max() > 1e-4:
        ...
        print(
            df_all.sort_values("diff_t2", ascending=False)[
                ["id", "t2_true", "t2_py", "t2_oct", "diff_t2"]
            ].head(5)
        )
...
def verify_mwf() -> None:
    ...
    for i in range(n_samples):
        ...
        s = m0 * (
            mwf_true[i] * np.exp(-te_ms / t2_myelin) + (1.0 - mwf_true[i]) * np.exp(-te_ms / t2_ie)
        )
        signals.append(s)
    signals = np.stack(signals)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["mono_t2", "vfa_t1", "b1_dam", "inversion_recovery", "mwf"],
    )
