...
- context: 旧配布物の完全削除と検証 / change: ユーザーが PyPI の旧 20 リリース (0.1.0〜1.0.0) を web UI から delete、GitHub 側は旧 17 リリースの資産 34 件を `gh release delete-asset` で削除（最初の実行はリポジトリ外のディレクトリで `gh release view` が失敗し空振りしていたため、`--repo SugimotoKohei/qmrpy` を明示した版で再実行）。検証結果: PyPI の releases は 2.0.0 のみ、旧版の per-version JSON は 404、`https://pypi.org/simple/qmrpy/` は 2.0.0 の 2 ファイルのみ、Python 3.11 環境で `qmrpy==1.0.0` は解決不能・`qmrpy` は 2.0.0 に解決。GitHub は旧 17 リリースの資産 0 件、v2.0.0 は資産 2 件で公開状態を維持。`files.pythonhosted.org` の旧ファイル直リンクは Fastly の immutable キャッシュ (`cache-control: max-age=365000000, immutable`, `x-cache: HIT`) のため一時的に HTTP 200 を返すが、どのインデックスからも到達不能 / reason: yank と draft 化では実体の配布が止まらないため、非商用限定コードの再頒布を完全に停止する必要があったため / artifact: PyPI project, GitHub Releases, `THIRD_PARTY_NOTICES.md`, `docs/memo.md` / next: CDN キャッシュが期限切れになるか確認する。長期間残る場合は PyPI support に purge を依頼する (TBD)
- context: 検証時の誤警報 / change: `python3 -m pip install --dry-run qmrpy` が "No matching distribution" を返したが、原因は system python が 3.9.6 で qmrpy の requires-python >=3.11 を満たさないためだった。Python 3.11 の隔離 venv で再検証し、2.0.0 が正常に解決されることを確認 / reason: 削除作業の影響と誤認しないよう原因を切り分けたため / artifact: `docs/memo.md` / next: なし
- context: 論文の著者情報の是正 / change: ORCID (0000-0003-2702-5235) の公開記録を照会したところ、氏名は一致したが所属が原稿の `Independent Researcher, Japan` と食い違い、employments に Kawasaki University of Medical Welfare (Assistant Professor) と Okayama Diagnostic Imaging Center が登録されていた。ユーザー確認のうえ、所属を `Department of Radiological Technology, Faculty of Health Science and Technology, Kawasaki University of Medical Welfare, Japan`、corresponding author の連絡先を `sugimoto@mw.kawasaki-m.ac.jp` に変更し、`paper_biorxiv.md` / `paper.md` / `CITATION.cff` (affiliation 追加) / `docs/biorxiv_submission.md` の4ファイルへ反映。旧記載の残存がないことを確認し PDF を再ビルド / reason: `Independent Researcher` は既存 `paper.md` から引き継いだ未検証の記載であり、bioRxiv は公開後の取り下げができないため投稿前に確定させる必要があったため / artifact: `paper_biorxiv.md`, `paper.md`, `CITATION.cff`, `docs/biorxiv_submission.md`, `output/paper_build/paper_biorxiv.pdf` / next: 所属機関のプレプリント内規（事前承認や届出の要否）の確認はユーザー判断 (TBD)
